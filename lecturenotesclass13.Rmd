...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-Jacobmiller1/manhattan.csv")
```
...
This confidence interval represents than in 95% of samples, the median price of 
a one bedroom Manhattan apartment will be between 2,162 dollars and 2,875 
dollars.
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_lived, level = .95)
ci_mice_lived
```
...
In a given sample of mice that were under the treatment, there is a 95% chance 
that the proportion of mice that lived will be between 0.207 and 0.310.
